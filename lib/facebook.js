...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data == 'error') return resolve({ creator: '@agusfamilia', status: false, message: 'tidak dapat diunduh' })
                resolve({ creator: '@agusfamilia', status: true, thumb: data.thumbnail, data: data.links })
            })
    })
}
