...
let handler = async (m, { conn }) => {
  ...
  try {
    ...
    if (!levelling.canLevelUp(user.level, user.exp, global.multiplier)) {
      ...
      rank.build()
        .then(async data => {
          await conn.sendButtonImg(m.chat, `Level *${user.level} (${user.exp - min}/${xp})*\nKurang *${max - user.exp}* lagi!`.trim(), data, 'Fokus Dot Id', 'AUTO LEVEL UP', ',on autolevelup')
        })
    }
    ...
    if (before !== user.level) {
      ...
      rank.build()
        .then(async data => {
          await conn.sendButtonImg(m.chat, `_*Level Up!*_\n_${before}_ -> _${user.level}_`.trim(), data, 'Fokus Dot Id', 'MATIKAN AUTO LEVEL UP', ',off autolevelup')
        })
    }
  }
}
