...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    if (m.chat in conn.casino) return m.reply ('Masih ada yang bermain casino disini, tunggu sampai selesai!!')
    ...
    try {
        ...
        if (global.db.data.users[m.sender].exp >= count * 1) {
            ...
            if (Aku > Kamu) {
                conn.reply(m.chat, `💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You LOSE*\nKamu kehilangan ${count} XP(xp)`.trim(), m)
            } else if (Aku < Kamu) {
                global.db.data.users[m.sender].exp += count * 2
                conn.reply(m.chat, `💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You Win*\nKamu mendapatkan ${count * 2} XP(xp)`.trim(), m)
            } else {
                global.db.data.users[m.sender].exp += count * 1
                conn.reply(m.chat, `💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*SERI*\nKamu mendapatkan ${count * 1} XP(xp)`.trim(), m)
            }
        } else conn.reply(m.chat, `XP(xp) kamu tidak mencukupi untuk bermain Casino silahkan ketik *#claim* untuk mengclaim harian anda!`.trim(), m)
    } catch (e) {
        console.log(e)
        m.reply('Error!!')
        if (DevMode) {
